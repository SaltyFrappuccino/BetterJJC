...
public class IncreaseOutputCNTFixProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if (world.getLevelData().getGameRules().getBoolean(BetterjjcModGameRules.BETTER_JJC_INCREASE_OUTPUT) == true) {
			if (entity.getPersistentData().getDouble("cnt6") <= 4) {
				entity.getPersistentData().putBoolean("BetterJJC_IncreaseOutput", false);
			}
		}
	}
}
