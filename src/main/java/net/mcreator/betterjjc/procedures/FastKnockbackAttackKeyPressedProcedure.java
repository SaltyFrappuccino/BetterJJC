...
public class FastKnockbackAttackKeyPressedProcedure {
		...
		if ((new Object() {
			...
			if ((entity.getPersistentData().getDouble("skill") == new Object() {
				double convert(String s) {
					...
					+ Math.round(1))) == entity instanceof LivingEntity && ((LivingEntity) entity).hasEffect(JujutsucraftModMobEffects.COOLDOWN_TIME_COMBAT.get())) {
				...
				}.convert(Math.round((entity.getCapability(JujutsucraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JujutsucraftModVariables.PlayerVariables())).PlayerCurseTechnique) + "" + Math.round(0) + Math.round(1)));
